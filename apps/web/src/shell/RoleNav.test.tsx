...
describe('RoleNav', () => {
  it('shows only inspection scanning for inspectors', () => {
    ...
    expect(screen.queryByText('生产计划')).toBeNull();
    expect(screen.queryByText('查询分析')).toBeNull();
    expect(screen.queryByText('基础数据')).toBeNull();
  });

  it('shows production plan and query analysis for query users', () => {
    render(<RoleNav role="QUERY" activeModule="productionPlan" onModuleChange={vi.fn()} />);

    expect(screen.queryByText('检验扫描')).toBeNull();
    expect(screen.getByText('生产计划')).toBeTruthy();
    expect(screen.getByText('查询分析')).toBeTruthy();
    expect(screen.queryByText('基础数据')).toBeNull();
  });

  it('shows all modules for administrators', () => {
